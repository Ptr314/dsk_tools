...
namespace dsk_tools {
    ...
    std::string ViewerBASIC::convert_tokenized(const BYTES & data, const std::string & cm_name, const std::array<const char*, 128> & tokens, const bool use_globals)
    {
        std::string out;
        bool in_rem = false;
        bool in_str = false;
        ...
        auto add_char = [&elements, &last, &in_str, &in_rem, &in_asm](std::string v) {
            ...
            if (v >= "0" && v <= "9") {
                if (last == EntityType::NUMBER || last == EntityType::VAR) {
                    TypeValue& lastPair = elements.back();
                    lastPair.second += v;
                } else {
                    last = EntityType::NUMBER;
