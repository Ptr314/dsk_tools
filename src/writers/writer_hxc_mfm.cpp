...
namespace dsk_tools {
    ...
    Result WriterHxCMFM::write(BYTES &buffer)
    {
        ...
        if (format_id == "FILE_HXC_MFM") {
            if (type_id == "TYPE_AGAT_140") {
                ...
                return Result::error(ErrorCode::WriteUnsupported, QT_TRANSLATE_NOOP("errors", "MFM format not supported for this disk type"));
        ...
        if (format_id == "FILE_MFM_NIB") {
            if (type_id == "TYPE_AGAT_140") {
                ...
                return Result::error(ErrorCode::WriteUnsupported, QT_TRANSLATE_NOOP("errors", "NIB format not supported for this disk type"));
        ...
        if (format_id == "FILE_MFM_NIC") {
            if (type_id != "TYPE_AGAT_140")
                return Result::error(ErrorCode::WriteUnsupported, QT_TRANSLATE_NOOP("errors", "NIC format not supported for this disk type"));
            ...
            return Result::error(ErrorCode::WriteUnsupported, QT_TRANSLATE_NOOP("errors", "Unknown MFM writer format"));
    ...
    {
        return Result::error(ErrorCode::WriteUnsupported, QT_TRANSLATE_NOOP("errors", "Track substitution not supported for MFM format"));
    }
